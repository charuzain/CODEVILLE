...
An array of recipe objects. Each recipe has a name property(string) and an ingredient property(array) We are limiting our search to two ingredient recipes. We want to find a recipe that utilizes one ingredient from Bakery A and one from Bakery B.
...
const chooseRecipe = (bakeryA,bakeryB,recipes)=>{
  for (let recipe of recipes) {
    const inBakeryA = recipe.ingredients.some(ingredient=>bakeryA.includes(ingredient));
    const inBakeryB = recipe.ingredients.some(ingredient=>bakeryB.includes(ingredient));
