...
const checkAir = (samples,threshold)=>{
  const value = samples.reduce((count , item)=>{
    if (item === 'dirty') {
      return count + 1;
    } else {
      return  count;
  ...
  return value / samples.length > threshold ? "Polluted" : "Clean";
};
const samples = ['clean', 'clean', 'clean', 'clean', 'dirty', 'clean', 'clean', 'dirty', 'clean', 'dirty'];
const threshold = 0.3;
